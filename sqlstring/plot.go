...
func plot_get() string {
	sql := " SELECT id, user_id, name, plot_type, code, area, ST_AsText(geo_field) as geo_field, lat, lng, address, area_type, detail, land_ownership, pics, doc_date, last_updated_time FROM plot "
	sql += " WHERE "
	return sql
}
