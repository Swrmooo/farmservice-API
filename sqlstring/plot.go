...
func plot_get() string {
	sql := " SELECT id, user_id, name, area, ST_AsText(geo_field) as geo_field, lat, lng, address, area_type, detail, land_ownership, pics, plot_type, doc_date, code FROM plot "
	sql += " WHERE "
	return sql
}
