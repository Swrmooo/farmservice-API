...
func writeLog(operation string, username string, key string, msg string, logfilename string, showDisplay bool){
	t := time.Now()
  ...
  defer file.Close()
	if showDisplay {
		fmt.Print("Log|o="+operation+"|u="+username+"|k="+key+"|m=["+msg+"]\r\n")
	}
  fmt.Fprintf(file, "t="+t.Format("15:04:05.000")+"|o="+operation+"|u="+username+"|k="+key+"|m=["+msg+"]\r\n")
}
