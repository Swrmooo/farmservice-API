...
func Risk_List(c *fiber.Ctx) error {
	...
	filters := lib.GetMask(r.Payload, []string{"risk_type", "icon_type", "user_id", "lat", "lng", "radius", "code"})
	filter := " id <> 0 "
...
func Risk_Update(c *fiber.Ctx) error {
	...
	payload := lib.GetMask(r.Payload, []string{"plot_id", "risk_type", "icon_type", "lat", "lng", "radius", "field"})
