...
func GetRequestToken(c *fiber.Ctx, conn string, operation string, allowAnonymous bool) *RequestToken {
	...
	if user != nil {
		lib.Log(operation, lib.T(user, "tel"), "", lib.MapToString(payload), "Operation")
	} else {
...
func (r *RequestToken) Success(info interface{},) error {
	...
	lib.LogHiddenWithDuration(r.Operation, tel, "Success", text, lib.I64_S(lib.DateTimeValueDiffSec(r.Time, time.Now())), "OperationSuccess")
